...
class ShortTermParkingSpot: ParkingSpot {
    ...
    init(id: String, address: Address, coordinates: Coordinate, pricePerHour: Double, provider: String, comments: String, tags: [String], firstName: String, lastName: String, lastEndTime: Int, times: [Int: [ParkingTimeInterval]], images: [String]) {
        self.times = times
        super.init(id: id, address: address, coordinates: coordinates, pricePerHour: pricePerHour, provider: provider, comments: comments, tags: tags, firstName: firstName, lastName: lastName, lastEndTime: lastEndTime, images: images)
        
    }
    // init from Full Days means that the parking spot is open for the full days in the array ( 0 -> Monday, 6 -> Sunday)
    init(id: String, address: Address, coordinates: Coordinate, pricePerHour: Double, provider: String, comments: String, tags: [String], firstName: String, lastName: String, lastEndTime: Int, fromFullDays: [Int], images: [String]) {
...
extension ShortTermParkingSpot {
    ...
    func compareHourMinutes(_ date1: Date, _ date2: Date) -> Int{
        if(Calendar.current.component(.hour, from: date1) == Calendar.current.component(.hour, from:date2)) {
            return Calendar.current.component(.minute, from: date1) - Calendar.current.component(.hour,from:date2)
        }
